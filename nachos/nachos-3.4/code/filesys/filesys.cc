...
FileSystem::Create(char *name, int initialSize)
{
    ...
    if (directory->Find(name) != -1)
      success = FALSE;			// file is already in directory
        ...
        sector = freeMap->FindFreeSlot();// find a sector to hold the file header
    	if (sector == -1) 		
...
{
    int freeslot = this->getIdxFreeSlot();
    if (freeslot == -1) {
        return NULL;
    }
...
int FileSystem::Open(char* name, int _type) {
    int freeslot;
    if (_type == 2) {
        freeslot = 0;
    } else if (_type == 3) {
    ...
    delete directory;
    if (openFile) {
        this->file_table[freeslot] = openFile;
        return freeslot;
    }
    return -1;
}
